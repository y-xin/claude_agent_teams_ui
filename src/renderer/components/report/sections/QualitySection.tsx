...
export const QualitySection = ({
  ...
  return (
    <ReportSection
      title={t('report.qualitySignals')}
      icon={BarChart3}
      defaultCollapsed={defaultCollapsed}
    >
      {/* Prompt quality */}
      <div className="mb-4">
        <div className="mb-2 text-xs font-medium text-text-muted">{t('report.promptQuality')}</div>
        ...
        <div className="mt-2 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <div>
            <div className="text-xs text-text-muted">{t('report.firstMessage')}</div>
            <div className="text-sm font-medium text-text">
              {prompt.firstMessageLengthChars.toLocaleString()} chars
            </div>
          </div>
          <div>
            <div className="text-xs text-text-muted">{t('report.userMessages')}</div>
            <div className="text-sm font-medium text-text">{prompt.userMessageCount}</div>
          </div>
          <div>
            <div className="text-xs text-text-muted">{t('report.corrections')}</div>
            <div className="text-sm font-medium text-text">{prompt.correctionCount}</div>
          </div>
          <div>
            <div className="text-xs text-text-muted">{t('report.frictionRate')}</div>
            <div className="text-sm font-medium text-text">
      ...
      <div className="mb-4">
        <div className="mb-2 flex items-center gap-2">
          <span className="text-xs font-medium text-text-muted">{t('report.startupOverhead')}</span>
          <AssessmentBadge assessment={startup.overheadAssessment} metricKey="startup" />
        </div>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          <div>
            <div className="text-xs text-text-muted">{t('report.messagesBeforeWork')}</div>
            <div className="text-sm font-medium text-text">{startup.messagesBeforeFirstWork}</div>
          </div>
          <div>
            <div className="text-xs text-text-muted">{t('report.tokensBeforeWork')}</div>
            <div className="text-sm font-medium text-text">
              {startup.tokensBeforeFirstWork.toLocaleString()}
            </div>
          </div>
          <div>
            <div className="text-xs text-text-muted">{t('report.pctOfTotal')}</div>
      ...
      <div className="mb-4">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          <div>
            <div className="text-xs text-text-muted">{t('report.totalReads')}</div>
            <div className="text-sm font-medium text-text">{fileReadRedundancy.totalReads}</div>
          </div>
          <div>
            <div className="text-xs text-text-muted">{t('report.uniqueFiles')}</div>
            <div className="text-sm font-medium text-text">{fileReadRedundancy.uniqueFiles}</div>
          </div>
          <div>
            <div className="text-xs text-text-muted">{t('report.readsPerUniqueFile')}</div>
            <div className="text-sm font-medium text-text">
      ...
      <div>
        <div className="mb-2 text-xs font-medium text-text-muted">
          {t('report.testProgression')}
        </div>
        <div className="mb-2 flex items-center gap-2">
          <AssessmentBadge assessment={testProgression.trajectory} metricKey="testTrajectory" />
          <span className="text-xs text-text-muted">
            {t('report.snapshotCount', { count: testProgression.snapshotCount })}
          </span>
        </div>
        {testProgression.firstSnapshot && testProgression.lastSnapshot && (
          <div className="grid grid-cols-2 gap-3">
            <div>
              <div className="text-xs text-text-muted">{t('report.firstRun')}</div>
              <div className="text-sm text-text">
                <span style={{ color: severityColor('good') }}>
                  {testProgression.firstSnapshot.passed} {t('report.passed')}
                </span>
                {' / '}
                <span style={{ color: severityColor('danger') }}>
                  {testProgression.firstSnapshot.failed} {t('report.failed')}
                </span>
              </div>
            </div>
            <div>
              <div className="text-xs text-text-muted">{t('report.lastRun')}</div>
              <div className="text-sm text-text">
                <span style={{ color: severityColor('good') }}>
                  {testProgression.lastSnapshot.passed} {t('report.passed')}
                </span>
                {' / '}
                <span style={{ color: severityColor('danger') }}>
                  {testProgression.lastSnapshot.failed} {t('report.failed')}
                </span>
              </div>
            </div>
          </div>
