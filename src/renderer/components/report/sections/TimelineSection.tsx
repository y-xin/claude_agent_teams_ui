...
export const TimelineSection = ({
  ...
  return (
    <ReportSection
      title={t('report.timelineActivity')}
      icon={Clock}
    ...
    >
      ...
      <div className="mb-4">
        <div className="mb-2 flex items-center gap-2">
          <span className="text-xs font-medium text-text-muted">{t('report.idleAnalysis')}</span>
          <AssessmentBadge assessment={idle.idleAssessment} metricKey="idle" />
        </div>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <div>
            <div className="text-xs text-text-muted">{t('report.idleGaps')}</div>
            <div className="text-sm font-medium text-text">{idle.idleGapCount}</div>
          </div>
          <div>
            <div className="text-xs text-text-muted">{t('report.totalIdle')}</div>
            <div className="text-sm font-medium text-text">{idle.totalIdleHuman}</div>
          </div>
          <div>
            <div className="text-xs text-text-muted">{t('report.activeTime')}</div>
            <div className="text-sm font-medium text-text">{idle.activeWorkingHuman}</div>
          </div>
          <div>
            <div className="text-xs text-text-muted">{t('report.idlePct')}</div>
        ...
        <div className="mb-4">
          <div className="mb-2 flex items-center gap-2">
            <span className="text-xs font-medium text-text-muted">
              {t('report.modelSwitches')} ({modelSwitches.count})
            </span>
      ...
      {keyEvents.length > 0 && (
        <div>
          <div className="mb-2 text-xs font-medium text-text-muted">{t('report.keyEvents')}</div>
          <div className="flex flex-col gap-1">
