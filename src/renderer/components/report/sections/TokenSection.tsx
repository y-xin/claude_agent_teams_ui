...
export const TokenSection = ({ data, cacheEconomics, defaultCollapsed }: TokenSectionProps) => {
  ...
  return (
    <ReportSection title={t('report.tokenUsage')} icon={Coins} defaultCollapsed={defaultCollapsed}>
      {/* By-model table */}
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-border text-left text-text-muted">
              ...
              <th className="pb-2 pr-4 text-right">{t('report.apiCalls')}</th>
              ...
              <th className="pb-2 pr-4 text-right">{t('report.cacheCreate')}</th>
          ...
          <tbody>
            ...
            <tr className="border-t border-border font-medium">
              <td className="py-1.5 pr-4 text-text">{t('report.total')}</td>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div>
          <div className="text-xs text-text-muted">{t('report.cacheEfficiency')}</div>
        ...
        <div>
          <div className="text-xs text-text-muted">{t('report.rwRatio')}</div>
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium text-text">
        ...
        <div>
          <div className="text-xs text-text-muted">{t('report.cacheReadPct')}</div>
          <div className="text-sm font-medium text-text">{data.totals.cacheReadPct}%</div>
        </div>
        <div>
          <div className="text-xs text-text-muted">{t('report.coldStart')}</div>
          <div
          ...
          >
            {cacheEconomics.coldStartDetected ? t('report.yes') : t('report.no')}
          </div>
        </div>
      </div>
    </ReportSection>
