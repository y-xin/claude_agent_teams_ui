  ...
  const assigneeField = (
    <div className="grid gap-2">
      <Label className={requiresOwner ? undefined : 'label-optional'}>
        {requiresOwner ? t('team.createTask.assignee') : t('team.createTask.assigneeOptional')}
      </Label>
      <MemberSelect
        members={members}
        value={owner || null}
        ...
        placeholder={
          requiresOwner
            ? t('team.createTask.selectMember')
            : t('team.createTask.selectMemberPlaceholder')
        }
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-[580px]">
        <DialogHeader>
          <DialogTitle>{t('team.createTask.title')}</DialogTitle>
          <DialogDescription>{t('team.createTask.description')}</DialogDescription>
        </DialogHeader>
        ...
        {!isTeamAlive ? (
          ...
          >
            ...
            <p className="text-xs leading-relaxed">{t('team.createTask.offlineNotice')}</p>
          </div>
        ) : null}

        <div className="grid gap-4 py-2">
          <div className="grid gap-2">
            <Label htmlFor="task-subject">{t('team.createTask.subject')}</Label>
            <Input
              id="task-subject"
              placeholder={t('team.createTask.subjectPlaceholder')}
              value={subject}
              autoFocus
          ...
          >
            ...
            <span>
              {showOptionalFields
                ? t('team.createTask.hideOptionalFields')
                : t('team.createTask.showOptionalFields')}
            </span>
          </button>
              ...
              <div className="grid gap-4">
                <div className="grid gap-2">
                  <Label className="label-optional">
                    {t('team.createTask.descriptionOptional')}
                  </Label>
                  <TiptapEditor
                    content={descriptionDraft.value}
                    onChange={descriptionDraft.setValue}
                    placeholder={t('team.createTask.descriptionPlaceholder')}
                    minHeight="100px"
                    maxHeight="200px"
                    toolbar
                  />
                ...
                <div className="grid gap-2">
                  <Label htmlFor="task-prompt" className="label-optional">
                    {t('team.createTask.promptOptional')}
                  </Label>
                  <MentionableTextarea
                    id="task-prompt"
                    placeholder={t('team.createTask.promptPlaceholder')}
                ...
                {availableTasks.length > 0 ? (
                  <div className="grid gap-2">
                    <Label className="label-optional">
                      {t('team.createTask.blockedByOptional')}
                    </Label>
                    <div className="overflow-hidden rounded-md border border-[var(--color-border)] bg-[var(--color-surface)]">
                      {availableTasks.length > 3 ? (
                        <div className="relative border-b border-[var(--color-border)] px-2 py-1.5">
                          ...
                          <input
                            type="text"
                            placeholder={t('team.createTask.searchTasksPlaceholder')}
                ...
                {availableTasks.length > 0 ? (
                  <div className="grid gap-2">
                    <Label className="label-optional">{t('team.createTask.relatedOptional')}</Label>
                    <div className="overflow-hidden rounded-md border border-[var(--color-border)] bg-[var(--color-surface)]">
                      {availableTasks.length > 3 ? (
                        <div className="relative border-b border-[var(--color-border)] px-2 py-1.5">
                          <Search
          ...
          {owner ? (
            <div className="grid gap-1">
              <div className="flex items-center gap-2">
                ...
                >
                  {t('team.createTask.startImmediately')}
                </Label>
              </div>
              {!isTeamAlive ? (
                <p className="text-[10px] text-[var(--color-text-muted)]">
                  {t('team.createTask.offlineStartNotice')}
                </p>
              ) : null}
            </div>
          ) : null}
