  ...
  return (
    <div className="flex items-center gap-3 border-b border-border bg-surface-sidebar px-4 py-2">
      ...
      <div className="flex items-center gap-1 text-xs text-text-muted">
        ...
        <span className="ml-1">
          {t('team.review.acrossFiles', { count: changeStats.filesChanged })}
        </span>
      </div>
      ...
      <Tooltip>
        ...
        <TooltipContent side="bottom">
          {autoViewed ? t('team.review.autoViewedOn') : t('team.review.autoViewedOff')}
        </TooltipContent>
      </Tooltip>
      ...
      {editedCount > 0 && (
        <span className="inline-flex items-center gap-1 rounded-full bg-amber-500/20 px-2 py-0.5 text-xs text-amber-400">
          <Pencil className="size-3" /> {t('team.review.editedCount', { count: editedCount })}
        </span>
      )}
      ...
      {canUndo && onUndo && (
        <Tooltip>
          ...
          <TooltipContent side="bottom">{t('team.review.undoTooltip')}</TooltipContent>
        </Tooltip>
      )}
        ...
        <>
          <Tooltip>
            <TooltipTrigger asChild>
              ...
              >
                <Check className="size-3" />
                {t('team.review.acceptAll')}
              </button>
            </TooltipTrigger>
            <TooltipContent side="bottom">{t('team.review.acceptAllTooltip')}</TooltipContent>
          </Tooltip>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              ...
              >
                <X className="size-3" />
                {t('team.review.rejectAll')}
              </button>
            </TooltipTrigger>
            <TooltipContent side="bottom">{t('team.review.rejectAllTooltip')}</TooltipContent>
          </Tooltip>
        </>
      )}
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            ...
            >
              ...
              {applying ? t('team.review.applying') : t('team.review.applyAllChanges')}
            </button>
          </TooltipTrigger>
          <TooltipContent side="bottom">{t('team.review.applyAllTooltip')}</TooltipContent>
        </Tooltip>
      )}
    </div>
  );
